...
/**
 * Dispatches orchestration commands for a client connection or a plugin: queued
 * behind server startup, fenced after `thread.create` so deletion cleanup of a
 * reused thread id finishes first, and running a `thread.turn.start` bootstrap
 * (thread creation, worktree, setup script, first turn).
 */
export class CommandDispatcher extends Context.Service<
  CommandDispatcher,
  {
    /**
     * Every command, including the server's own bootstrap sub-commands, carries
     * `origin` when given: the requester caused them.
     */
    readonly dispatch: (
...
>()("t3/orchestration/CommandDispatcher") {}
...
export const make = Effect.gen(function* () {
  ...
  const forOrigin = (origin: OrchestrationClientOrigin | undefined) => {
  ...
  return CommandDispatcher.of({
    dispatch: (command, origin) => forOrigin(origin)(command),
  });
});

export const layer = Layer.effect(CommandDispatcher, make);
