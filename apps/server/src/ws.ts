...
import * as PluginThreads from "./plugins/PluginThreads.ts";
...
const makeWsRpcLayer = (
  ...
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      const commandDispatcher = yield* CommandDispatcher.CommandDispatcher;
      const dispatchNormalizedCommand = (command: OrchestrationCommand) =>
        commandDispatcher.dispatch(command, clientOrigin);
...
export const websocketRpcRouteLayer = Layer.unwrap(
        ...
        const rpcWebSocketHttpEffect = yield* Effect.gen(function* () {
          ...
          Effect.provide(
            makeWsRpcLayer(
              ...
              Layer.provide(Layer.succeed(CommandDispatcher.CommandDispatcher, commandDispatcher)),
  ...
  // One dispatcher serves client connections and plugins.
  Layer.provide(
    PluginPackageManager.layer.pipe(
      Layer.provideMerge(PluginCommandCatalog.layer),
      Layer.provideMerge(PluginThreads.layer),
      Layer.provideMerge(CommandDispatcher.layer),
    ),
  ),
);
